...
- **`memory/`** — Persistent memory files at repo root (NOT under `.Codex/`). Contains `MEMORY.md` (index) and `project_experiment_workflow.md` (authoritative experiment workflow). Tracked in git.
- **`.Codex/settings.local.json`**: Grants permissions for `Bash(node *)`, `Bash(git *)`, and `mcp__ide__getDiagnostics` (VS Code language diagnostics integration).
- **`.Codex/agents/prd-analyst.md`**: Custom agent definition for PRD analysis — use via the Agent tool with `subagent_type: "prd-analyst"` when generating product requirement documents from codebase analysis.
